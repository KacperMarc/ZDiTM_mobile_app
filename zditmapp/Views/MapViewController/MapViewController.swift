...
class MapViewController: UIViewController {
    ...
    lazy var toggleButtonsStackView: UIStackView = {
        let buttonHeight: CGFloat = 35 // Mniejsze niż cały stack
       ...
       /* let vehiclesButton = UIButton(type: .system)
        vehiclesButton.setTitle("Pojazdy", for: .normal)
        vehiclesButton.titleLabel?.font = UIFont.systemFont(ofSize: 14, weight: .light)
        vehiclesButton.addTarget(self, action: #selector(toggleVehicles), for: .touchUpInside)
        vehiclesButton.backgroundColor = backgroundColor
        vehiclesButton.tintColor = .black
        vehiclesButton.layer.cornerRadius = cornerRadius
        vehiclesButton.layer.masksToBounds = true
        vehiclesButton.translatesAutoresizingMaskIntoConstraints = false
        vehiclesButton.configuration = .gray()
        
        let stopsButton = UIButton(type: .system)
        stopsButton.setTitle("Przystanki", for: .normal)
        stopsButton.titleLabel?.font = UIFont.systemFont(ofSize: 14, weight: .light)
        stopsButton.addTarget(self, action: #selector(toggleStops), for: .touchUpInside)
        stopsButton.backgroundColor = backgroundColor
        stopsButton.tintColor = .black
        stopsButton.layer.cornerRadius = cornerRadius
        stopsButton.layer.masksToBounds = true
        ...
        */
        ...
        stack.spacing = 10
        stack.alignment = .center // Kluczowe! Przyciski będą miały mniejszą wysokość niż stack
        stack.translatesAutoresizingMaskIntoConstraints = false
        
        // Ustawienie wysokości przycisków i labela
    ...
    private func addVehicleAnnotations() async throws {
        do{
            //możnaby po kliknięciu w daną linie dodać w wysuwanym menu taką opcje, aby pokazać trasę i pojazdy tej linii
    ...
    func updateMapView() {
        if viewModel.showVehicles {
            ...
            Task {
                do {
                    ...
                    print("Błąd podczas dodawania linii: \(error)")
                }
            }
        }else {
        ...
        if viewModel.showStops {
            Task {
                do {
                    //renew request musi byc wydzielony w pizdu daleko aby nie mial wplywu na adnotacjie przystankow bo z dodaniem pojazdow zaczyna dodawac cyklicznie te same przystanki
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            // Ograniczenia dla searchVehicleTextField
            NSLayoutConstraint.activate([
    ...
    //obsługa zatrzymania aktualizacji po wyjściu z widoku
